-- Function to recalculate leaderboard_stats from gift_cards
CREATE OR REPLACE FUNCTION recalculate_leaderboard_stats()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
  existing_zns_domains RECORD;
BEGIN
  -- Save existing ZNS domains before truncating
  CREATE TEMP TABLE IF NOT EXISTS temp_zns_domains AS
  SELECT sender_address, zns_domain
  FROM leaderboard_stats
  WHERE zns_domain IS NOT NULL AND zns_domain != '';
  
  -- Delete all existing stats
  TRUNCATE TABLE leaderboard_stats;
  
  -- Recalculate from gift_cards
  INSERT INTO leaderboard_stats (
    user_identifier,
    sender_address,
    social_platform,
    cards_sent_total,
    amount_sent_total,
    amount_sent_by_currency,
    last_sent_at,
    last_recipient,
    display_name,
    avatar_url,
    zns_domain
  )
  WITH card_events AS (
    SELECT
      LOWER(TRIM(sender_address)) AS sender_address,
      amount::numeric AS amount,
      currency,
      recipient_username,
      created_at
    FROM gift_cards
    WHERE sender_address IS NOT NULL 
      AND TRIM(sender_address) != ''
      AND LENGTH(TRIM(sender_address)) >= 10  
  ),
  currency_totals AS (
    SELECT
      sender_address,
      jsonb_object_agg(currency, currency_sum) AS amount_sent_by_currency
    FROM (
      SELECT
        sender_address,
        currency,
        SUM(amount) AS currency_sum
      FROM card_events
      GROUP BY sender_address, currency
    ) t
    GROUP BY sender_address
  ),
  last_activity AS (
    SELECT DISTINCT ON (sender_address)
      sender_address,
      recipient_username AS last_recipient,
      created_at AS last_sent_at
    FROM card_events
    ORDER BY sender_address, created_at DESC
  ),
  aggregated AS (
    SELECT
      sender_address,
      COUNT(*) AS cards_sent_total,
      SUM(amount) AS amount_sent_total
    FROM card_events
    GROUP BY sender_address
  )
  SELECT
    a.sender_address AS user_identifier,
    a.sender_address,
    'address' AS social_platform,
    a.cards_sent_total,
    a.amount_sent_total,
    COALESCE(ct.amount_sent_by_currency, '{}'::jsonb),
    la.last_sent_at,
    la.last_recipient,
    NULL::text AS display_name,
    NULL::text AS avatar_url,
    tzd.zns_domain
  FROM aggregated a
  LEFT JOIN currency_totals ct ON ct.sender_address = a.sender_address
  LEFT JOIN last_activity la ON la.sender_address = a.sender_address
  LEFT JOIN temp_zns_domains tzd ON LOWER(TRIM(tzd.sender_address)) = LOWER(TRIM(a.sender_address));
  
  -- Update updated_at timestamp for all records (after INSERT)
  -- This ensures all records have the same updated_at timestamp
  -- Note: WHERE TRUE is required by PostgreSQL for UPDATE statements
  UPDATE leaderboard_stats
  SET updated_at = NOW()
  WHERE TRUE;
  
  -- Clean up temp table
  DROP TABLE IF EXISTS temp_zns_domains;
END;
$$;

-- Grant execute permission
GRANT EXECUTE ON FUNCTION recalculate_leaderboard_stats() TO authenticated;
GRANT EXECUTE ON FUNCTION recalculate_leaderboard_stats() TO anon;

